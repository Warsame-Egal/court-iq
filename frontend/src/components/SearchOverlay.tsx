...
export default function SearchOverlay() {
  ...
  const overlayBg =
    theme.palette.mode === 'dark' ? 'rgba(0, 0, 0, 0.92)' : 'rgba(255, 255, 255, 0.98)';
  ...
  const doSearch = useCallback(async (q: string) => {
    ...
    try {
      const data = await fetchJson<SearchResults>(
        `${API_BASE_URL}/api/v1/search?q=${encodeURIComponent(q.trim())}`,
        {},
      ...
      <DialogContent sx={{ p: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        >
          <TextField
            ...
            inputProps={{ 'aria-label': 'Search players and teams' }}
            sx={{
              '& .MuiInputBase-root': { fontSize: '1.25rem', color: 'text.primary' },
          ...
          <IconButton
            ...
            size="large"
            sx={{ minWidth: 44, minHeight: 44, color: 'text.primary' }}
          >
            <Close />
          </IconButton>
        </Box>
