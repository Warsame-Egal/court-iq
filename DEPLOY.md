...
**HTTPS:** Cloudflare quick tunnels via `nohup` on the VM (background).

Full numbered runbook: copy [`DEPLOY.local.md.example`](DEPLOY.local.md.example) → `DEPLOY.local.md` (gitignored).

## First time

1. GCP VM (e2-micro, Ubuntu, firewall 8080/8000)
2. VM: clone repo, `.env`, `docker compose -f docker-compose.prod.yml up -d --build`
3. VM: `nohup cloudflared …` for `:8080` and `:8000` (see example doc §3)
4. Vercel: `frontend/`, set `VITE_API_BASE_URL` + `VITE_WS_URL`
...
Tunnels + Vercel env: update only when `cloudflared` restarts (new `trycloudflare.com` URLs).
