...
class SeasonLeadersCategory(BaseModel):
    ...
    category: str = Field(
        ..., description="Stat category name (e.g., 'Points Per Game')"
    )
    leaders: List[SeasonLeader] = Field(
        ..., description="Top 5 players in this category"
    )
...
class SeasonLeadersResponse(BaseModel):
    ...
    categories: List[SeasonLeadersCategory] = Field(
        ..., description="Leaders for each stat category"
    )
