...
async def get_team_player_stats(
    team_id: int, season: Optional[str] = None
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if season_start_year > current_year or (
                    ...
                    return TeamPlayerStatsResponse(
                        team_id=team_id, season=season, players=[]
                    )
        ...
        if stats_data.empty:
            logger.warning(
                f"No player stats data returned from NBA API for season {season}"
            )
        ...
        if "TEAM_ID" not in stats_data.columns:
            logger.error(
                f"TEAM_ID column not found in player stats data for season {season}"
            )
            return TeamPlayerStatsResponse(team_id=team_id, season=season, players=[])
        ...
        logger.error(
            f"Timeout fetching team player stats for team {team_id}, season {season}"
        )
        raise HTTPException(
            status_code=503,
        ...
        error_msg = str(e)
        logger.error(
            f"Error fetching team player stats for team {team_id}, season {season}: {e}",
            exc_info=True,
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching team player stats: {error_msg}"
        )
