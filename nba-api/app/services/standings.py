...
async def getSeasonStandings(season: str, max_retries: int = 3) -> StandingsResponse:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if df.empty:
                logger.warning(
                    f"No standings data returned from NBA API for season {season}"
                )
                raise HTTPException(
            ...
            for team in teams_data:
                ...
                try:
                    ...
                    ppg_raw = (
                        team.get("PointsPG") or team.get("PTS") or team.get("Points")
                    )
                    opp_ppg_raw = (
                        team.get("OppPointsPG")
                        or team.get("OPP_PTS")
                        or team.get("OppPoints")
                    )
                    ...
                    logger.debug(
                        f"Could not extract PPG/OPP PPG for team {team.get('TeamID')}: {e}"
                    )
                ...
                try:
                    ...
                    logger.error(
                        f"Error creating StandingRecord for team {team.get('TeamID', 'unknown')}: {e}"
                    )
                    raise
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(
                    f"Failed to fetch standings for {season} after {max_retries} attempts: {e}"
                )
