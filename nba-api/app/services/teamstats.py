...
async def get_team_stats(season: Optional[str] = None) -> TeamStatsResponse:
    ...
    try:
        ...
        try:
            ...
            if (
                ...
                raise HTTPException(
                    status_code=404, detail=f"No data available for season {season}"
                )
            raise
        ...
        if "PLUS_MINUS" in stats_data.columns:
            stats_data["NET_RTG"] = pd.to_numeric(
                stats_data["PLUS_MINUS"], errors="coerce"
            ).fillna(0)
        ...
        def create_category(
            ...
            for team in teams_data:
                ...
                value = (
                    float(team.get(stat_col, 0))
                    if pd.notna(team.get(stat_col))
                    else 0.0
                )
        ...
        categories = []
        categories.append(
            create_category(stats_data, "NET_RTG", "Net Rating", top_n=30)
        )
        ...
        logger.error(
            f"Error fetching team stats for season {season}: {e}", exc_info=True
        )
        raise HTTPException(
