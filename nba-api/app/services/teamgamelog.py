...
async def get_team_game_log(
    team_id: str, season: Optional[str] = None
) -> TeamGameLogResponse:
    """
    ...
    try:
        ...
        try:
            ...
            if not game_finder_data.empty:
                ...
                for row in games_data:
                    try:
                        game_entry = TeamGameLogEntry(
                            ...
                            game_date=(
                                pd.to_datetime(row.get("GAME_DATE", "")).strftime(
                                    "%Y-%m-%d"
                                )
                            ...
                            win_loss=(
                                str(row.get("WL", ""))
                                if pd.notna(row.get("WL"))
                                and str(row.get("WL", "")).strip()
                                else None
                            ),
                            points=(
                                int(row.get("PTS", 0))
                            ...
                            field_goals_made=(
                                int(row.get("FGM", 0))
                                if pd.notna(row.get("FGM"))
                                else None
                            ),
                            field_goals_attempted=(
                                int(row.get("FGA", 0))
                                if pd.notna(row.get("FGA"))
                                else None
                            ),
                            field_goal_pct=(
                                float(row.get("FG_PCT", 0))
                                if pd.notna(row.get("FG_PCT"))
                                else None
                            ),
                            three_pointers_made=(
                                int(row.get("FG3M", 0))
                                if pd.notna(row.get("FG3M"))
                                else None
                            ),
                            three_pointers_attempted=(
                                int(row.get("FG3A", 0))
                                if pd.notna(row.get("FG3A"))
                                else None
                            ),
                            three_point_pct=(
                                float(row.get("FG3_PCT", 0))
                                if pd.notna(row.get("FG3_PCT"))
                                else None
                            ),
                            free_throws_made=(
                                int(row.get("FTM", 0))
                                if pd.notna(row.get("FTM"))
                                else None
                            ),
                            free_throws_attempted=(
                                int(row.get("FTA", 0))
                                if pd.notna(row.get("FTA"))
                                else None
                            ),
                            free_throw_pct=(
                                float(row.get("FT_PCT", 0))
                                if pd.notna(row.get("FT_PCT"))
                                else None
                            ),
                            turnovers=(
                                int(row.get("TOV", 0))
                                if pd.notna(row.get("TOV"))
                                else 0
                            ),
                            steals=(
                                int(row.get("STL", 0))
                                if pd.notna(row.get("STL"))
                                else 0
                            ),
                            blocks=(
                                int(row.get("BLK", 0))
                                if pd.notna(row.get("BLK"))
                                else 0
                            ),
                        )
                ...
                return TeamGameLogResponse(
                    team_id=team_id_int, season=season, games=games
                )
        except Exception as e:
            logger.warning(
        ...
        if not game_log_data.get("resultSets") or len(game_log_data["resultSets"]) == 0:
            logger.warning(
                f"No resultSets in game log data for team {team_id}, season {season}"
            )
        ...
        logger.info(
            f"Found {len(game_log)} games in game log for team {team_id}, season {season}"
        )
        ...
        logger.info(
            f"Successfully parsed {len(games)} games for team {team_id}, season {season}"
        )
        ...
        error_msg = str(e)
        logger.error(
            f"Error fetching game log for team {team_id}, season {season}: {e}",
            exc_info=True,
        )
        ...
        try:
            ...
            if season_start_year > current_year or (
                ...
                return TeamGameLogResponse(
                    team_id=int(team_id), season=season, games=[]
                )
        ...
        if (
            ...
            or "resolve" in error_msg.lower()
        ):
            raise HTTPException(
                status_code=503,
                detail="Unable to connect to NBA API. Please try again later.",
            )

        raise HTTPException(
            status_code=500, detail=f"Error fetching game log: {error_msg}"
        )
