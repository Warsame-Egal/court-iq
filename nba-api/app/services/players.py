...
from app.schemas.seasonleaders import (
    ...
    try:
        ...
        player_index_data = await asyncio.wait_for(
            asyncio.to_thread(
                lambda: playerindex.PlayerIndex(
                    historical_nullable=HistoricalNullable.all_time, **api_kwargs
                )
            ),
            timeout=15.0,
        )
        ...
        try:
            ...
            game_log_data = await asyncio.wait_for(
                asyncio.to_thread(
                    lambda: PlayerGameLog(player_id=player_id, **api_kwargs).get_dict()
                ),
                timeout=10.0,
            )

            if (
                not game_log_data.get("resultSets")
                or len(game_log_data["resultSets"]) == 0
            ):
                ...
                for row in game_log[:5]:
                    try:
                        game_id_idx = (
                            game_headers.index("Game_ID")
                            if "Game_ID" in game_headers
                            else None
                        )
                        game_date_idx = (
                            game_headers.index("GAME_DATE")
                            if "GAME_DATE" in game_headers
                            else None
                        )
                        matchup_idx = (
                            game_headers.index("MATCHUP")
                            if "MATCHUP" in game_headers
                            else None
                        )
                        pts_idx = (
                            game_headers.index("PTS") if "PTS" in game_headers else None
                        )
                        reb_idx = (
                            game_headers.index("REB") if "REB" in game_headers else None
                        )
                        ast_idx = (
                            game_headers.index("AST") if "AST" in game_headers else None
                        )
                        stl_idx = (
                            game_headers.index("STL") if "STL" in game_headers else None
                        )
                        blk_idx = (
                            game_headers.index("BLK") if "BLK" in game_headers else None
                        )
                        ...
                        if None in [
                            ...
                            logger.warning(
                                f"Missing required headers in game log for player {player_id}"
                            )
                            continue
                        ...
                        recent_games.append(
                            PlayerGamePerformance(
                                ...
                                date=pd.to_datetime(row[game_date_idx]).strftime(
                                    "%Y-%m-%d"
                                ),
                                opponent_team_abbreviation=(
                                    row[matchup_idx][-3:]
                                    if len(row[matchup_idx]) >= 3
                                    else ""
                                ),
                        ...
                        logger.warning(
                            f"Error parsing game log row for player {player_id}: {e}"
                        )
                        continue
        except Exception as e:
            # If game log fails, continue without recent games rather than failing the whole request
            logger.warning(
                f"Error fetching game log for player {player_id}: {e}. Continuing without recent games."
            )
...
async def get_season_leaders(season: Optional[str] = None) -> SeasonLeadersResponse:
    """
    ...
    try:
        ...
        for col in numeric_cols:
            if col in stats_data.columns:
                stats_data[col] = pd.to_numeric(
                    stats_data[col], errors="coerce"
                ).fillna(0)
        ...
        def create_leaders(
            ...
            for row in leaders_data:
                ...
                value = (
                    float(row.get(stat_col, 0)) if pd.notna(row.get(stat_col)) else 0.0
                )
                ...
                if value >= 0 and player_name:
                    ...
                    if is_percentage:
                        formatted_value = (
                            round(value * 100, 1) if value <= 1 else round(value, 1)
                        )
        ...
        categories.append(
            create_leaders(
                stats_data, "FG_PCT", "Field Goal Percentage", is_percentage=True
            )
        )
        categories.append(
            create_leaders(
                stats_data, "FG3M", "Three Pointers Made", is_whole_number=True
            )
        )
        categories.append(
            create_leaders(
                stats_data, "FG3_PCT", "Three Point Percentage", is_percentage=True
            )
        )
        ...
        logger.error(
            f"Error fetching season leaders for season {season}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail=f"Error fetching season leaders: {str(e)}"
        )
...
async def get_league_roster() -> List[PlayerSummary]:
    ...
    try:
        ...
        if "ROSTER_STATUS" in player_index_df.columns:
            try:
                ...
                numeric_mask = pd.to_numeric(
                    player_index_df["ROSTER_STATUS"], errors="coerce"
                ).notna()
                ...
                if numeric_mask.any():
                    ...
                    player_index_df["ROSTER_STATUS"] = player_index_df[
                        "ROSTER_STATUS"
                    ].fillna("")
                    active_players = player_index_df[
                        player_index_df["ROSTER_STATUS"]
                        .astype(str)
                        .str.upper()
                        .str.strip()
                        == "ACTIVE"
                ...
                if active_players.empty:
                    logger.warning(
                        "No players with ROSTER_STATUS='Active' found. Returning all players."
                    )
                    active_players = player_index_df
            except Exception as e:
                logger.warning(
                    f"Error filtering by ROSTER_STATUS: {e}. Returning all players."
                )
                active_players = player_index_df
        else:
        ...
        for player_data in players_data:
            try:
                ...
                player_summaries.append(
                    PlayerSummary(
                        PERSON_ID=(
                            int(player_data.get("PERSON_ID", 0))
                            if pd.notna(player_data.get("PERSON_ID"))
                            else 0
                        ),
                        PLAYER_LAST_NAME=_safe_str(
                            player_data.get("PLAYER_LAST_NAME"), ""
                        ),
                        PLAYER_FIRST_NAME=_safe_str(
                            player_data.get("PLAYER_FIRST_NAME"), ""
                        ),
                        PLAYER_SLUG=_safe_str(player_data.get("PLAYER_SLUG")),
                        TEAM_ID=_safe_int(player_data.get("TEAM_ID")),
                        TEAM_SLUG=_safe_str(player_data.get("TEAM_SLUG")),
                        IS_DEFUNCT=_safe_int(player_data.get("IS_DEFUNCT")),
                        TEAM_CITY=_safe_str(player_data.get("TEAM_CITY")),
                        TEAM_NAME=_safe_str(player_data.get("TEAM_NAME")),
                        TEAM_ABBREVIATION=_safe_str(
                            player_data.get("TEAM_ABBREVIATION")
                        ),
                        JERSEY_NUMBER=_safe_str(player_data.get("JERSEY_NUMBER")),
                        POSITION=_safe_str(player_data.get("POSITION")),
                        HEIGHT=_safe_str(player_data.get("HEIGHT")),
                        WEIGHT=_safe_int(player_data.get("WEIGHT")),
                        COLLEGE=_safe_str(player_data.get("COLLEGE")),
                        COUNTRY=_safe_str(player_data.get("COUNTRY")),
                        ROSTER_STATUS=roster_status or "",
                        PTS=_safe_float(player_data.get("PTS")),
                        REB=_safe_float(player_data.get("REB")),
                        AST=_safe_float(player_data.get("AST")),
                        STL=_safe_float(player_data.get("STL")),
                        BLK=_safe_float(player_data.get("BLK")),
                        STATS_TIMEFRAME=_safe_str(player_data.get("STATS_TIMEFRAME")),
                        FROM_YEAR=_safe_int(player_data.get("FROM_YEAR")),
                        TO_YEAR=_safe_int(player_data.get("TO_YEAR")),
                        recent_games=[],
                    )
                )
            except Exception as e:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching league roster: {str(e)}"
        )
...
async def get_player_game_log(
    ...
    try:
        ...
        for row in game_log:
            try:
                ...
                game_entry = PlayerGameLogEntry(
                    ...
                    game_date=pd.to_datetime(get_value("GAME_DATE", "")).strftime(
                        "%Y-%m-%d"
                    ),
                    matchup=str(get_value("MATCHUP", "")),
                    win_loss=(
                        str(val) if (val := get_value("WL", None)) is not None else None
                    ),
                    minutes=(
                        str(val)
                        if (val := get_value("MIN", None)) is not None
                        else None
                    ),
                    points=get_int("PTS"),
        ...
        return PlayerGameLogResponse(
            player_id=player_id_int, season=season, games=games
        )
        ...
        logger.error(
            f"Error fetching game log for player {player_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail=f"Error fetching game log: {str(e)}"
        )
